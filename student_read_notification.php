...
if (isset($_POST['messages'])) {
    ...
    foreach ($messageIds as $messageId) {
        ...
        $checkQuery = "SELECT * FROM notification_read WHERE student_id = '$session_id' AND notification_id = '$messageId'";
        $result = mysqli_query($conn, $checkQuery);
        $count = mysqli_num_rows($result);

        if ($count == 0) {
            // The notification hasn't been marked as read by the student, so insert the record
            mysqli_query($conn, "INSERT INTO notification_read (student_id, student_read, notification_id) VALUES ('$session_id', 'yes', '$messageId')") or die(mysqli_error($conn));
        }
    }
